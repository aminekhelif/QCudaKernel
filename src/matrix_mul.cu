...
__global__ void matMulKernel(const baseType* __restrict__ A,
                             ...
                             int M, int N, int K) 
{
    ...
    if (row<M && col<N) {
        C[row*N+col] = toBaseType(accum);
    }
}

extern "C" void launchMatMulKernel(const baseType* d_A, const baseType* d_B, baseType* d_C,
                                   int M, int N, int K, cudaStream_t stream) {
    dim3 blockDim(TILE_SIZE, TILE_SIZE);
    dim3 gridDim((N+TILE_SIZE-1)/TILE_SIZE, (M+TILE_SIZE-1)/TILE_SIZE);
    matMulKernel<<<gridDim, blockDim, 0, stream>>>(d_A, d_B, d_C, M, N, K);
}